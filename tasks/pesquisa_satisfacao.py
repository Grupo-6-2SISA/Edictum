        ...
        for atendimento_concluido in atendimentos_concluídos:
            ...
            try:
                ...
                db.executarQuery(
                    ...
                    f"(null, {atendimento_concluido['id_cliente']}, null, null, 1, DEFAULT, NOW(), 'Sucesso ao enviar email de pesquisa de satisfação para {nome} ({email})')")
            except Exception as e:
                print(f"[ERRO] Falha ao enviar email para {nome} ({email}): {str(e)}")
                db.executarQuery(
                    "INSERT INTO log_envio_lembrete (fk_atendimento, fk_cliente, fk_conta, fk_nota_fiscal, funcionou, id_log_envio_lembrete, data_hora_criacao, mensagem) VALUES "
                    f"(null, {atendimento_concluido['id_cliente']}, null, null, 0, DEFAULT, NOW(), 'Falha ao enviar email de pesquisa de satisfação para {nome} ({email}): {str(e)}')")
                continue
