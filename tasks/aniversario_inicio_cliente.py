...
def run():
    ...
    if not clientes:
        print("[INFO] Nenhum cliente faz 1 ano como cliente hoje.")
        return

    for cliente in clientes:
        nome_cliente = cliente['nome']
        data_inicio = cliente['data_inicio'].strftime('%d/%m/%Y')
        assunto = f"Agradecemos seu 1º ano como cliente!"
        mensagem = f"""
        Prezado(a) {nome_cliente},

        Hoje celebramos o seu primeiro ano como nosso cliente, iniciado em {data_inicio}!
        ...
        try:
            ...
            db.executarQuery(
                ...
                f"(6, null, {cliente['id_cliente']}, null, null, 1, DEFAULT, NOW(), 'Sucesso ao enviar email de 1 ano como cliente para {nome_cliente}')"
            )
            ...
            db.executarQuery(
                "INSERT INTO log_envio_lembrete (fk_tipo_lembrete, fk_atendimento, fk_cliente, fk_conta, fk_nota_fiscal, funcionou, id_log_envio_lembrete, data_hora_criacao, mensagem) VALUES "
                f"(6, null, {cliente['id_cliente']}, null, null, 0, DEFAULT, NOW(), 'Falha ao enviar email de 1 ano como cliente para {nome_cliente}: {erro_sql}')"
            )